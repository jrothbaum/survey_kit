...
df_joined = df_hhld.join(df_person, how="inner", left_on=["h_seq"], right_on=["ph_seq"])
df_joined = compress_df(
    df_joined.join(df_rep_weights, on=["h_seq", "pppos"], how="inner"),
    check_string=True,
)
...
replicates = Replicates(weight_stub="pwwgt", df=df_joined)
