"""
Same convention as lgbm_parameter_effects.py, for Parameters.RandomForest()
...
def _build_and_run(df, variable, path_suffix, index="idx"):
    srmi = SRMI(
        ...
        storage=SRMI.Storage(
            path_model=f"{path_scratch}/py_rf_param_effects_{path_suffix}", force_start=True
        ),
    )
    srmi.run()
    return srmi
...
#   Deliberately overfittable (deep, unregularized trees) so in-sample vs.
#       out-of-fold predictions are guaranteed to diverge
overfit_rf_params = dict(n_estimators=50, max_depth=None, min_samples_leaf=1)
...
v_cv_off = Variable(
    ...
    parameters=Parameters.RandomForest(
        parameters=overfit_rf_params, error=Parameters.ErrorDraw.pmm, cv_folds=0
    ),
)
...
v_cv_on = Variable(
    ...
    modeltype=Variable.ModelType.RandomForest,
    parameters=Parameters.RandomForest(
        parameters=overfit_rf_params, error=Parameters.ErrorDraw.pmm, cv_folds=5
    ),
)
...
v_pmm = Variable(
    ...
    model=["x1"],
    modeltype=Variable.ModelType.RandomForest,
    parameters=Parameters.RandomForest(error=Parameters.ErrorDraw.pmm),
)
...
v_random = Variable(
    ...
    model=["x1"],
    modeltype=Variable.ModelType.RandomForest,
    parameters=Parameters.RandomForest(error=Parameters.ErrorDraw.Random),
)
...
v_leaf = Variable(
    ...
    model=["x1"],
    modeltype=Variable.ModelType.RandomForest,
    parameters=Parameters.RandomForest(error=Parameters.ErrorDraw.leaf),
)
...
assert "Extracting leaf indices" in log_leaf, (
    "error=leaf should log that it's extracting tree leaf indices for donor matching"
)
imputed_leaf = _collect(srmi_leaf).filter(pl.Series(miss_e))["y"].to_list()
assert all(v in observed_e_values for v in imputed_leaf), (
    "error=leaf should also only ever donate real observed values (leaf co-occurrence "
    "matching, not a predicted value)"
)
...
#   RandomForest's own prediction noise (unlike OLS's smooth fit) dilutes
#       the shrinkage effect somewhat - 0.85 is still a clearly-meaningful
#       improvement, just not as tight a bound as the OLS/Regression version
...
#   ============================================================
#   4) No native categorical handling - documented limitation. A raw
#      string predictor via list-form should fail; the documented
#      workaround (a formula with C(...)) should work.
#   ============================================================
logger.info("=== RandomForest has no native categorical handling (documented limitation) ===")

rng4 = np.random.default_rng(20260910)
n4 = 1000
cat_levels = {"a": 0.0, "b": 5.0, "c": -5.0}
cat = rng4.choice(list(cat_levels.keys()), size=n4)
x1_cat = rng4.normal(size=n4)
y_cat = x1_cat + np.array([cat_levels[c] for c in cat]) + rng4.normal(scale=1.0, size=n4)
df_cat = pl.DataFrame(dict(idx=range(n4), x1=x1_cat, cat=cat, y=y_cat))
miss_cat = rng4.random(n4) < 0.2
df_cat = df_cat.with_columns(
    pl.when(pl.Series(miss_cat)).then(None).otherwise(pl.col("y")).alias("y")
)

v_cat_list_form = Variable(
    impute_var="y",
    model=["x1", "cat"],
    modeltype=Variable.ModelType.RandomForest,
    parameters=Parameters.RandomForest(error=Parameters.ErrorDraw.pmm),
...
try:
    _build_and_run(df_cat, v_cat_list_form, "cat_list_form_crash")
    raise AssertionError(
        "expected a raw string predictor via list-form to fail for RandomForest "
        "(no native categorical handling) - if this now works, the documented "
        "limitation may have changed and this test should be updated"
    )
except AssertionError:
    raise
except Exception as e:
    logger.info(f"Correctly rejected (no native categorical handling): {type(e).__name__}: {e}")

v_cat_formula_form = Variable(
    impute_var="y",
    model="~1+x1+C(cat)",
    modeltype=Variable.ModelType.RandomForest,
    parameters=Parameters.RandomForest(error=Parameters.ErrorDraw.pmm),
)
srmi_cat_formula = _build_and_run(df_cat, v_cat_formula_form, "cat_formula_form")
logger.info("RandomForest categorical limitation: PASSED (crashes raw, works via C(...))")

logger.info("randomforest_parameter_effects.py: all checks passed")
