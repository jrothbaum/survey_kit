...
| Regression across implicates (statsmodels/linearmodels/pyfixest/polars_ds/R/Stata) | `mi_ses_from_<package>()` - see the [Regression Adapters guide](adapters.md) | Same shape as `mi_ses_from_function()`, combined across implicates via Rubin's rules |
